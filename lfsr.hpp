...
public:
    ...
    void next(SAMPLE input=0) {
        ...
        for (int i=m-1; i>0; i--) {
            m_state[i] = (m_state[i-1] + m_v*m_K[i]) % static_cast<u16>(p);
        }
        m_state[0] = (input + m_v*m_K[0]) % static_cast<u16>(p);
#endif
    }
    void back(SAMPLE input=0) {
        const SAMPLE m_v = (m_inv_K0*(m_state[0] - input + p)) % static_cast<u16>(p);
        for (int i=0; i<m-1; i++) {
            m_state[i] = (m_state[i+1] - m_v*m_K[i+1] + p*p) % static_cast<u16>(p);
        }
        m_state[m-1] = m_v;
    }
